    ...
    useEffect(() => {
        const fetchSearchedItems = async () => {
            try {
                const apiKey = process.env.REACT_APP_API_KEY;
                const apiUrl = process.env.REACT_APP_API_URL;
                const response = await axios.get(`${apiUrl}/Item?&keyW=${searchQuery}`, {
                    headers: {
                        'APIKey' : apiKey
                    }
                });
                    ...
                    items.map((item) => (
                        <div className='w-full h-[400px] p-3 flex flex-col shadow-sm hover:shadow-lg duration-200 rounded-tr-[40px] rounded-bl-[40px]'>
                            ...
                            <div className='w-full h-[220px] flex flex-col items-center justify-center'>
                                ...
                                <button className='w-[90%] mx-auto font-roboto rounded-tr-3xl rounded-bl-3xl my-2 py-1 shadow hover:bg-yellow hover:text-white hover:shadow-lg duration-200'>Add to cart</button>
                            </div>
                        </div>
                    )) : (
