    ...
    <div className='w-full min-h-screen pt-28'>
        ...
        <div className='flex w-[90%] mx-auto mt-10 flex-col md:flex-row'>
            ...
            <div className='w-full md:w-[400px] h-auto md:h-[500px] p-4'>
                ...
                <div className='w-full h-full overflow-y-auto mt-5'>
                    {categories.map((category) => (
                        <div key={category.categoryMainID} className='w-full flex items-center justify-between cursor-pointer' onClick={() => {fetchItems(category.categoryMainID); setActiveCategory(category.categoryMainID);}}>
            ...
            <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-5 md:mt-0'>
                {items.length > 0 ? 
                    items.map((item) => (
                        <Link key={item.itemID} to={`/product/${item.itemID}`} className='w-full h-[400px] p-3 flex flex-col shadow-sm hover:shadow-lg duration-200 rounded-tr-[40px] rounded-bl-[40px]'>
                            ...
                            <div className='w-full h-[220px] flex flex-col items-center justify-center'>
                                ...
                                <button 
                                    ...
                                    className='w-[90%] font-roboto mx-auto rounded-tr-3xl rounded-bl-3xl mt-2 py-1 shadow hover:bg-yellow hover:text-white hover:shadow-lg duration-200'>Add to cart</button>
                            </div>
                        </Link>
                    )) : (
